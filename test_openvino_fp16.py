...
ie = Core()
model_encoder = ie.compile_model(model=ie.read_model(model='encoder.fp16.onnx'), device_name="CPU")
...
n_layer_self_k_cache = np.empty((len(model.decoder.blocks), n_audio, model.dims.n_text_ctx, model.dims.n_text_state), np.float16)
n_layer_self_v_cache = np.empty((len(model.decoder.blocks), n_audio, model.dims.n_text_ctx, model.dims.n_text_state), np.float16)
offset = np.zeros(1, dtype=np.int64)

model_decoder = ie.compile_model(model=ie.read_model(model='decoder.fp16.onnx'), device_name="CPU")
