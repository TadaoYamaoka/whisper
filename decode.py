...
class DecodingTask:
    ...
    def __init__(self, model: "Whisper", options: DecodingOptions):
        self.encoder_session = onnxruntime.InferenceSession('encoder.onnx', providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        self.decoder_session = onnxruntime.InferenceSession('decoder.onnx', providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        self.model = model
    ...
    def _get_audio_features_onnx(self, mel: Tensor):
    ...
    @torch.no_grad()
    def _detect_language_onnx(self, mel: Tensor, tokenizer: Tokenizer, n_layer_cross_k, n_layer_cross_v) -> Tuple[Tensor, List[dict]]:
        """
    ...
    def _detect_language(self, audio_features: Tensor, tokens: Tensor, n_layer_cross_k, n_layer_cross_v):
        languages = [self.options.language] * audio_features.shape[0]
        lang_probs = None

        if self.options.language is None or self.options.task == "lang_id":
            lang_tokens, lang_probs = self.model.detect_language(audio_features, self.tokenizer)
            print(lang_tokens, lang_probs)
            lang_tokens, lang_probs = self._detect_language_onnx(audio_features, self.tokenizer, n_layer_cross_k, n_layer_cross_v)
            print(lang_tokens, lang_probs)
    ...
    def _main_loop_onnx(self, audio_features: Tensor, tokens: Tensor, n_layer_cross_k, n_layer_cross_v):
        assert audio_features.shape[0] == tokens.shape[0]
        n_batch = tokens.shape[0]
        sum_logprobs: Tensor = torch.zeros(n_batch, device=audio_features.device)
        no_speech_probs = [np.nan] * n_batch

        n_layer_self_k_cache = onnxruntime.OrtValue.ortvalue_from_shape_and_type((len(self.model.decoder.blocks), n_batch, self.model.dims.n_text_ctx, self.model.dims.n_text_state), element_type=np.float32, device_type='cuda', device_id=0)
        n_layer_self_v_cache = onnxruntime.OrtValue.ortvalue_from_shape_and_type((len(self.model.decoder.blocks), n_batch, self.model.dims.n_text_ctx, self.model.dims.n_text_state), element_type=np.float32, device_type='cuda', device_id=0)
        try:
            for i in range(self.sample_len):
                logits = self.inference.logits(tokens, audio_features)
                # onnxruntime
    ...
    def run(self, mel: Tensor) -> List[DecodingResult]:
        ...
        n_layer_cross_k, n_layer_cross_v = self._get_audio_features_onnx(mel)
        tokens: Tensor = torch.tensor([self.initial_tokens]).repeat(n_audio, 1)

        # detect language if requested, overwriting the language token
        languages, language_probs = self._detect_language(audio_features, tokens, n_layer_cross_k, n_layer_cross_v)
        ...
        tokens, sum_logprobs, no_speech_probs = self._main_loop_onnx(audio_features, tokens, n_layer_cross_k, n_layer_cross_v)
